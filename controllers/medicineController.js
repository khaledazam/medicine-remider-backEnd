...
export const addMedicine = async (req, res) => {
  try {
    const { name, dosage, schedule, notes, quantity, refillAlertAt, slot } = req.body;
...
export const markDoseTaken = async (req, res) => {
  try {
    ...
    if (alreadyTaken) {
      console.log("Dose already marked as taken today");
    ...
    const alerts = medicines
      ...
      .map(m => ({
        ...
        message: m.quantity === 0 
          ...
          : `Your stock for ${m.name} is low (${m.quantity} left, refill at ${m.refillAlertAt || 5}).`
      }));
  ...
  try {
    ...
    const summary = medicines.map(med => {
      ...
      return {
        ...
        missed: missed,
        adherence: adherence,
        status: adherence === 100 ? "Perfect" : adherence >= 50 ? "Partial" : "Poor",
    ...
    res.json({
      ...
      dailyStats: {
        ...
        status: overallAdherence === 100 ? "Perfect Day!" : overallAdherence >= 50 ? "Good Progress" : " Need Improvement"
      }
    });
